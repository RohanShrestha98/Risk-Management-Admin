...
import { useCategoryMutation, useChapterMutation, useCourseMutation, useInstructorMutation, useLiveGroupMutation, useLiveMutation, useNotificationMutation, useQuestionBankMutation, useQuestionMutation, useQuizMutation, useReferalCodeMutation, useSubjectMutation, useTestSeriesMutation, useTestTypeMutation, useUnitMutation } from "@/hooks/useMutateData";
...
export default function DeleteModal({ asChild, children, title, desc, id }) {
    ...
    const instructorsMutation = useInstructorMutation()
    ...
    const deleteMutation = pathname === "test-series" ? testSeriesMutation : pathname === "test-type" ? testTypeMutation : pathname === "test" ? quizMutation : pathname === "units" ? unitMutation : pathname === "live" ? liveMutation : pathname === "live-group" ? liveGroupMutation : pathname === "notification" ? notificationMutation : pathname === "chapters" ? chapterMutation : pathname === "instructors" ? instructorMutation : pathname === "courses" ? courseMutation : pathname === "subjects" ? subjectMutation : pathname === "category" ? categoryMutation : pathname === "instructors" ? instructorsMutation : pathname === "question-bank" ? questionBankMutation : pathname === "question" ? questionMutation : pathname === "referal-codes" ? referalCodesMutation : courseMutation
