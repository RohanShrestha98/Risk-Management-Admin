...
export default function AddInstructorModal({ asChild, children, edit = false, editData }) {
    ...
    const [selectedGender, setSelectedGender] = useState(edit ? editData?.gender : "")
            ...
            <DialogContent className="sm:max-w-[425px]  min-w-[500px] bg-[#FAFAFA]">
                ...
                <form onSubmit={handleSubmit(onSubmitHandler)}>
                    <div className="flex flex-col gap-4">
                        <ChooseImage setSelectedImage={setSelectedImage} selectedImage={selectedImage} defaultUrl={editData?.image} />
                        <div className="grid grid-cols-2 gap-2">
                            <div>
                                <CustomSelect options={courseOptions} label={""} placeholder={edit ? editData?.course?.courseID : "Select course"} className={"w-full text-sm text-gray-500"} labelName={"Course"} required={true} setSelectedField={setSelectedCourse} />
                            ...
                            <div>
                                <CustomSelect options={subjectOptions} label={""} placeholder={edit ? editData?.subject?.title : "Select subject"} className={"w-full text-sm text-gray-500"} labelName={"Subject"} required={true} setSelectedField={setSelectedSubject} />
                                <p className="text-red-600 text-xs">
                        ...
                        <div>
                            <CustomSelect options={genderOptions} label={""}
                                placeholder={edit ? editData?.gender : "Select gender"}
                                setSelectedField={setSelectedGender}
                                className={"w-full text-sm text-gray-500"} labelName={"Gender"} required={true} />
                            <p className="text-red-600 text-xs">
                        ...
                        <div className="grid grid-cols-2 gap-2">
                            <div className="">
                                <InputField register={register} required name="email" type="email" placeholder="Enter email" className="w-full text-sm text-gray-500" defaultValue="" label="Email" />
                                <p className="text-red-600 text-xs">
                                    {errors?.email?.message ?? error?.email}
                                </p>
                            </div>
                            <div className="">
                                <InputField register={register} name="phone" type="number" placeholder="Enter phone number" className="w-full text-sm text-gray-500" defaultValue="" required label="Phone Number" />
                                <p className="text-red-600 text-xs">
                    ...
                    <div className="grid grid-cols-2 w-full mt-10 gap-2">
                        <Button buttonName={`${edit ? "Reset" : "Clear"}`} className={"w-full "} danger handleButtonClick={(e) => handleClear(e)} icon={""} />
                        <Button type="submit" buttonName={`${edit ? "Edit" : "Add"} instructor`} handleButtonClick={() => { setHasSubmittedClick(true) }} className={"w-full"} icon={""} />
                    </div>
                </form>
            </DialogContent>
        </Dialog>
