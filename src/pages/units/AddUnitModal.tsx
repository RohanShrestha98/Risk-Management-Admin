...
export default function AddUnitModal({ asChild, children, edit, editData }) {
    ...
    const onSubmitHandler = async (data) => {
        ...
        try {
            const response = await unitMutation.mutateAsync([edit ? "patch" : "post", edit ? `update/${editData?.id}` : "create/", postData])
            setOpen(false)
            reset()
            setError()
    ...
    return (
        <Dialog open={open} onOpenChange={setOpen}>
            ...
            <DialogContent className="sm:max-w-[425px]  min-w-[500px] bg-[#FAFAFA]">
                <DialogTitle className="text-[#22244D] font-medium text-base">{edit ? "Edit" : "Add"} Unit</DialogTitle>
                <form onSubmit={handleSubmit(onSubmitHandler)}>
                    <div className="flex flex-col gap-4">
                        ...
                        <div className="">
                            <InputField register={register} name="title" placeholder="Enter Unit Name" className="w-full text-sm text-gray-500" defaultValue="" required label="Unit Name" />
                            <p className="text-red-600 text-xs">
                    ...
                    <div className="grid grid-cols-2 w-full mt-10 gap-2">
                        ...
                        <Button type="submit" buttonName={`${edit ? "Edit" : "Add"} Unit`} handleButtonClick={() => setHasSubmittedClick(true)} className={"w-full"} icon={""} />
                    </div>
                </form>
            </DialogContent>
        </Dialog>
